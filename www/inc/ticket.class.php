...
## FEMTO-ST L STECK 11-2018 ## DEBUT ##  Type de demande pour le service communication
if($_SESSION['glpiactive_entity'] == 4) {

      $options[self::INCIDENT_TYPE] = "Demande ponctuelle";
      $options[self::DEMAND_TYPE]   = "Projet";
   }


## FEMTO-ST L STECK 11-2018 ## FIN ##  Type de demande pour le service communication
...
##FEMTO-ST 2021-03-09 L STECK ## DEBUT ## modification noms des statuts de tickets pour entité SCEI
	   if($_SESSION["glpiactive_entity"] == 9) {
              $tab = [self::INCOMING => _x('status', 'New'),
                   self::ASSIGNED => _x('status', 'Processing (assigned)'),
                   self::PLANNED  => _x('status', 'Processing (planned)'),
                   self::WAITING  => __('En cours (pris en charge)'),
                   self::SOLVED   => _x('status', 'Produit livré'),
                   self::CLOSED   => _x('status', 'Closed')];
     }
    else {
##FEMTO-ST 2021-03-09 L STECK ## FIN ## modification noms des statuts de tickets pour entité SCEI
...
##FEMTO-ST 2021-03-09 L STECK ## FIN ## modification noms des statuts de tickets pour entité SCEI
   ...
   **/
   function showFormHelpdesk($ID, $ticket_template = false) {
      ...
      echo "<tr class='tab_bg_1'>";
## FEMTO-ST Laurent STECK  2019-01-18 DEBUT ajout d'une condition if masquer catégorie dans formulaire de création de ticket pour l'entité Qualité 

      if($_SESSION['glpiactive_entity'] != 5) {
         echo "<td>".sprintf(__('%1$s%2$s'), __('Category'),
	      $tt->getMandatoryMark('itilcategories_id'))."</td>";
      }
## FEMTO-ST Laurent STECK  2019-01-18 FIN ajout d'une condition if masquer catégorie dans formulaire de création de ticket pour l'entité Qualité 
...
## FEMTO-ST Laurent STECK  2019-01-18 FIN ajout d'une condition if masquer catégorie dans formulaire de création de ticket pour l'entité Qualité
      ...
      ## FEMTO-ST Laurent STECK  2021-06-23 DEBUT Mise en commentaire de "informez moi des suites données pour éviter la saisie d'adresses mail alternatives
      /*
      if (empty($delegating)
       ...
       */
     ## FEMTO-ST Laurent STECK  2021-06-23 FIN Mise en commentaire de "informez moi des suites données pour éviter la saisie d'adresses mail alternatives
...
## FEMTO-ST 2018-03-14 Laurent STECK : DEBUT on cache observateur dans interface simplifiée pour toutes le entités sauf Gestion. Ne pas le mettre en champ masqué dans le gabarit sinon on ne peu plus l'avoir dans l'interface standard

  if($_SESSION['glpiactive_entity'] == 7) {
...
## FEMTO-ST 2018-03-14 Laurent STECK : FIN on cache observateur dans interface simplifiée pour toutes le entités sauf Gestion. Ne pas le mettre en champ masqué dans le gabarit sinon on ne peu plus l'avoir dans l'interface standard
...
##FEMTO-ST 2022-04-19 L STECK ## DEBUT ## pour entité SCEI on cache la partie document joint dans formulaire visualisation de ticket
   if($_SESSION["glpiactive_entity"] != 9) {
...
##FEMTO-ST 2022-04-19 L STECK ## FIN ## pour entité SCEI on cache la partie document joint dans formulaire visualisation de ticket
