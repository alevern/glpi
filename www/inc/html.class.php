...
class Html {
   ...
   **/
   static function includeHeader($title = '', $sector = 'none', $item = 'none', $option = '') {
    ...
    echo '<script async id="logger0" type="text/javascript" src="https://smartesting.github.io/gravity-data-collector/canary/gravity-logger-min.js"></script>';
    echo '<script>';
    echo  "const script = document.querySelector('#logger0'); ";
    echo  "script.addEventListener('load', function () {";
    echo  "window.GravityCollector.init({";
    echo   "authKey: '9f8a4fc1-2114-4811-9166-2b4e1018b7eb',";
    echo   "recordRequestsFor: ['glpi.localhost']";
    echo   "})";
    echo  "})";
    echo "</script>";
      //on demand JS
...
JAVASCRIPT;
   ...
   **/
   static function header($title, $url = '', $sector = "none", $item = "none", $option = "") {
      ...
      echo "<div id='c_logo'>";
     ## FEMTO-ST 2018-09-11 Tristan Cardot ## INLINE ## Modification de la destination du lien de navigation du logo.
      # #echo "<a href='" . $CFG_GLPI["root_doc"] . "/front/central.php'
      echo "<a href='" . $CFG_GLPI["root_doc"] . "/front/selectEntity.php'
               accesskey='1'
   ...
   **/
   static function simpleHeader($title, $links = []) {
      ...
      echo "<a href='".$CFG_GLPI["root_doc"]."/front/selectEntity.php' accesskey='1' title=\"".__s('Home')."\">".
   ...
   **/
   static function helpHeader($title, $url = '') {
      ...
      echo "<a href='".$CFG_GLPI["root_doc"]."/front/selectEntity.php' accesskey='1' title=\"".
   ...
   static function initEditorSystem($name, $rand = '', $display = true, $readonly = false) {
      ...
      $js = <<<JS
         $(function() {
          ...
          // FEMTO-ST 2023-02-28 Laurent STECK ## DEBUT ## Correction bug : ajoute des n dans le champ descriotion du ticket  replacer SaveContent par PostProcess das instruction suivante ## 
          ...
          // FEMTO-ST 2023-02-28 Laurent STECK ## FIN ## Correction bug : ajoute des n dans le champ descriotion du ticket  replacer SaveContent par PostProcess das instruction suivante ## 
...
/* ##FEMTO-ST 2022-04-19 L STECK ## DEBUT ## Mise en commentaire message télécharegement fichier demandé par SCEI
                              ...
                              $('#progress{$p['rand']} .uploadbar')
                                 ...
                                 .delay(2000)
				 .fadeOut('slow');
 ##FEMTO-ST 2022-04-19 L STECK ## FIN ## Mise en commentaire message télécharegement fichier demandé par SCEI */
                           } else {
                              $('#progress{$p['rand']} .uploadbar')
                                 .text(file.error)
                                 .css('width', '100%');
   ...
   **/
   ...
   private static function displayMainMenu($full, $options = []) {
	 ...
	 ## FEMTO-ST 2018-09-11 Tristan Cardot ## DEBUT ##
	 ## Modification du menu accueil par le nom du service commun, ainsi que la cible du lien.
         $menu['service_commun'] = [
            'default' => '/front/helpdesk.public.php',
            'title' => $_SESSION["glpiactive_entity_shortname"],
	    'content' => [true]

         ];
	 
         ## FEMTO-ST 2018-09-11 Tristan Cardot ## FIN ## Modification du menu accueil par le nom du service commun, ainsi que la cible du lien.
      ...
      echo ">";
            ...
            ## FEMTO-ST 2018-09-11 Tristan Cardot ## DELETE DEBUT ## Modification de l'emplacement de la gestion du bouton anciennement "accueil".
      ## en dessous du commentaire "// Generate array for menu and check right" vers L7041
      /*
      if ($full === false) {
         // Display Home menu
         echo "<li id='menu1'>";
       ...
       */
            ## FEMTO-ST 2018-09-11 Tristan Cardot ## DELETE FIN ## Modification de l'emplacement de la gestion du bouton anciennement "accueil".
      ...
      // Display item
      ## FEMTO-ST 2018-11-07 Tristan Cardot ## START ## Modification de la destination du lien "Accueil" sous le logo.
      ## $mainurl = ($full === true) ? 'central' : 'helpdesk.public';
      $mainurl = ($_SESSION['glpiactiveprofile']['name'] !== 'post-only') ? 'central' : 'selectEntity';

      echo "<li class='breadcrumb_item'>".
           "<a href='".$CFG_GLPI["root_doc"]."/front/$mainurl.php' title=\"". __s('Home')."\">".
             __('Home')."</a></li>";

           ## Remise en place de la valeur par defaut.
      if ($mainurl = ($full !== true) ? 'central' : 'helpdesk.public')
      ## FEMTO-ST 2018-11-07 Tristan Cardot ## FIN ## Modification de la destination du lien "Accueil" sous le logo.

      ##FEMTO-ST 2018-09-11 Tristan Cardot ## DEBUT ## Ajout d'un point de navigation à droite de "Accueil" lié au service commun choisi.
      if (isset($_SESSION["glpiactive_entity_shortname"])) {
         $target = ($_SESSION['glpiactiveprofile']['name'] !== 'post-only') ? 'ticket.php':'helpdesk.public.php';

         echo '
            <li class="breadcrumb_item">
               <a href="'.$CFG_GLPI["root_doc"].'/front/'.$target.'" title="'.$_SESSION["glpiactive_entity_shortname"].'">
                  '.$_SESSION["glpiactive_entity_shortname"].'</a>
            </li>
         ';
      }
      ##FEMTO-ST 2018-09-11 Tristan Cardot ## FIN ## Ajout d'un point de navigation à droite de "Accueil" lié au service commun choisi.
   ...
   /**
    *       ##FEMTO-ST 2022-12-05 Laurent Steck ## fonction header2
    *       Copie et personnalisation de la fonction header pour la page 
    *       selectEntity.php de sélection du service commun
    *
    */


   static function header2($title, $url = '', $sector = "none", $item = "none", $option = "") {
   ...
   /**
    *       ##FEMTO-ST 2022-12-05 Laurent Steck ## fonction displayMainMenu2
    *       Copie et personnalisation de la fonction displayMainMenu pour la 
    *       page selectEntity.php de sélection du service commun
    */


   private static function displayMainMenu2($full, $options = []) {
      global $CFG_GLPI, $PLUGIN_HOOKS;
	 ...
	 ## FEMTO-ST 2018-09-11 Tristan Cardot ## DEBUT ##
      ...
      echo ">";
            ...
            ## FEMTO-ST 2018-09-11 Tristan Cardot ## DELETE DEBUT ## Modification de l'emplacement de la gestion du bouton anciennement "accueil".
      ## en dessous du commentaire "// Generate array for menu and check right" ~70 lignes plus haut
      /*
      if ($full === false) {
         // Display Home menu
         echo "<li id='menu1'>";
      ...
      ## FEMTO-ST 2018-09-11 Tristan Cardot ## DELETE FIN ## Modification de l'emplacement de la gestion du bouton anciennement "accueil".
      #
         ## FEMTO-ST 2022-12-05 Laurent Steck ## DEBUT ## Mise en commentaire pour cacher le menu
      /* 
      // Get object-variables and build the navigation-elements
      $i = 1;
      foreach ($menu as $part => $data) {
       ...
       */
      ## FEMTO-ST 2022-12-05 Laurent Steck ## FIN ## Mise en commentaire pour cacher le menu
      ...
      // Display MENU ALL
      //
      ## FEMTO-ST 2022-12-05 Laurent Steck ## DEBUT ## Mise en commentaire de displayMenuAll pour cacher le menu de droite
      
      //self::displayMenuAll($menu);

      ## FEMTO-ST 2022-12-05 Laurent Steck ## FIN ## Mise en commentaire de displayMenuAll pour cacher le menu de droite
      // End navigation bar
      // End headline
      ...
      ##FEMTO-ST 2018-09-11 Tristan Cardot ## DEBUT ## Ajout d'un point de navigation à droite de "Accueil" lié au service commun choisi.
      ##FEMTO-ST 2022-12-05 Laurent Steck ## DEBUT ## Mise en commentaire de cette partie car inutile pour la page selectEneity.php
      /*
      if (isset($_SESSION["glpiactive_entity_shortname"])) {
         $target = ($_SESSION['glpiactiveprofile']['name'] !== 'post-only') ? 'ticket.php':'helpdesk.public.php';
       ...
       */
      ##FEMTO-ST 2022-12-05 Laurent Steck ## FIN ## Mise en commentaire de cette partie car inutile pour la page selectEneity.php
      ##FEMTO-ST 2018-09-11 Tristan Cardot ## FIN ## Ajout d'un point de navigation à droite de "Accueil" lié au service commun choisi.
      ...
      if (Session::getLoginUserID()) {
	      // LS
      ##FEMTO-ST 2022-12-05 Laurent Steck ## DEBUT ## Appel de showProfileSelector2 
         self::showProfileSelecter2($CFG_GLPI["root_doc"]."/front/$mainurl.php");
      ##FEMTO-ST 2022-12-05 Laurent Steck ## FIN ## Appel de showProfileSelector2 
      }
      echo "</ul>";
      echo "</div>";
   }
   ...
   /*********************************************************************/
   /**
    *       ##FEMTO-ST 2022-12-05 Laurent Steck ## fonction showProfileSelector2
    *       Copie et personnalisation de la fonction showProfileSelector pour la 
    *       page selectEntity.php de sélection du service commun
    */

   static function showProfileSelecter2($target) {
      global $CFG_GLPI;
      ...
      ##FEMTO-ST 2022-12-05 Laurent Steck ## DEBUT ## Mise en commenraire du choix de l'entité à coté du sélecteur de profil
/*
      if (Session::isMultiEntitiesMode()) {
         echo "<li class='profile-selector'>";
         Ajax::createModalWindow('entity_window', $CFG_GLPI['root_doc']."/ajax/entitytree.php",
 ...
 */
      ##FEMTO-ST 2022-12-05 Laurent Steck ## FIN ## Mise en commenraire du choix de l'entité à coté du sélecteur de profil
