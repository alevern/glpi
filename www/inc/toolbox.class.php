            ...
            switch (Session::getCurrentInterface()) {
               case "helpdesk" :
                  switch (strtolower($data[0])) {
                     ...
                     case "tracking" :
		     case "ticket" :
                        //FEMTO-ST 2018-12-20 Tristan Cardot ## DEBUT ## Changement de profile si le paramètre N°4 est présent.
                        if (isset($data[3]))
                           Session::changeProfile($data[3]);
                        //FEMTO-ST 2018-12-20 Tristan Cardot ## FIN ## Changement de profile si le paramètre N°4 est présent.
   ...
   **/
   static function checkValidReferer() {
      ...
      if (!isset($url['host'])
         ...
         if ($_SESSION['glpi_use_mode'] == Session::DEBUG_MODE) {
            Html::displayErrorAndDie($_SERVER['HTTP_X_FORWARDED_SERVER'],//__("None or Invalid host in HTTP_REFERER. Reload previous page before doing action again."),
                                  true);
            $isvalidReferer = false;
         }
      }
